...
public class ManhattanEnhancedStrategy implements Strategy {
  @Override
  public BoardAction righAction(Board board) {
    ...
    for (BoardAction action : board.getAvailableActions()) {
      Integer dist = board.isHoleAction(action) ? Integer.MAX_VALUE
          : computeDistance(action, board.getCursor(), board.getGoalCursor());
      if (dist < min) {
        min = dist;
        bestAction = action;
      }
